...
Result<RecordId> TableIndexer::insert_row(TableSchema& schema, const std::vector<Value>& fields) {
    ...
    for (size_t i = 0; i < schema.columns.size(); ++i) {
        ...
        // Ключ уже есть в B+ дереве — блокируем вставку.
        // Ошибку возвращаем через Result/Status, как и остальной индексный слой:
        // исключение пролетало бы мимо отката уже применённых индексов ниже.
