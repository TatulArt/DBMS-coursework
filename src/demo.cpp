...
//   4. выполнение выборок через index вместо полного скана;
...
int main() {
    ...
    // Схема таблицы с явной инициализацией имен полей, чтобы не путать nullable
    TableSchema users;
    users.table_name = "users";
    users.columns = {
        ColumnDef{"id",   ColumnType::INT,    /*notNull*/ true,  /*indexed*/ true}, 
        ColumnDef{"name", ColumnType::STRING, /*notNull*/ true,  /*indexed*/ true},
        ColumnDef{"age",  ColumnType::INT,    /*notNull*/ false, /*indexed*/ false} 
    };
    ...
    try {
        ...
        std::cout << "INSERT с существующим id=1        -> ЗАБЛОКИРОВАНО: " << e.what() << "\n";
    }
    ...
    try {
        ...
        std::cout << "INSERT с существующим name        -> ЗАБЛОКИРОВАНО: " << e.what() << "\n";
    }

    try {
        auto null_key = indexer.insert_row(users, {val::Null(), Value("без id"), Value(30)}); 
        std::cout << "INSERT с NULL в INDEXED-колонке   -> " 
                  << (null_key.ok() ? "ПРИНЯТО (ошибка!)" : null_key.status().message) << "\n";
    } catch (const TypeError& e) {
        std::cout << "INSERT с NULL в INDEXED-колонке   -> ЗАБЛОКИРОВАНО: " << e.what() << "\n";
    }
    ...
    try {
        ...
        std::cout << "INSERT со строкой вместо INT      -> ЗАБЛОКИРОВАНО: " << e.what() << "\n";
    }
    ...
    // ------------------------------------------------------------------
    print_header("ЭТАП 5. Удаление записи и чистка index");
    // ------------------------------------------------------------------

    if (!by_id.value().records.empty()) {
        const RecordId victim = by_id.value().records.at(0).id; 
    ...
    const std::vector<PageId> saved_data_pages = users.data_pages;
    
    // Принудительно вызываем метод save() твоего друга для записи каталога на диск
    index_manager.save(); 
    page_manager.close();
    ...
    // Восстанавливаем схему и список страниц таблицы
    TableSchema restored = users;
    restored.data_pages = saved_data_pages;
    ...
    // Проверяем структурную целостность проиндексированных деревьев
    auto int_tree = reopened_indexes.get_index(TableIndexer::index_name_for("users", "id"));
    if (!int_tree.ok()) return fail("получение дерева index по id", int_tree.status());

    auto str_tree = reopened_indexes.get_string_index(TableIndexer::index_name_for("users", "name"));
    if (!str_tree.ok()) return fail("получение дерева index по name", str_tree.status());
    ...
    for (auto it = int_tree.value().begin(); it != int_stop; ++it) {
        int_keys++;
    }

    size_t str_keys = 0;
    const StringIndexIterator str_stop = str_tree.value().end();
    for (auto it = str_tree.value().begin(); it != str_stop; ++it) {
        str_keys++;
    }
