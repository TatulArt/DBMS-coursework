...
public:
    ...
    // Выделение страницы: сначала берётся из списка свободных страниц,
    // и только если он пуст — файл расширяется новой страницей в конце.
    Status allocate_page(PageId& new_page_id, Page& page_out);

    // Возврат страницы в список свободных. Страница помечается служебной
    // подписью, поэтому повторный free_page() той же страницы не создаёт
    // петлю в списке. Нулевую страницу (метаданные) освободить нельзя.
    Status free_page(PageId page_id);

    // Сколько страниц лежит в списке свободных (нужно тестам и диагностике)
    Result<uint32_t> free_page_count() const;
